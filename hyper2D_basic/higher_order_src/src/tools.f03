module tools
  ...
  subroutine export_sol_vtk(t_ID, U)
    ...
    real(kind=8), dimension(Neq,Nx,Ny) :: prim ! Primitive variables
    ...
    do i = 1, Nx
      do j = 1, Ny
        call compute_primitive_from_conserved(U(:,i,j), prim(:,i,j))
      end do
    end do
    ...
    do ID_EQ = 1, Neq ! Loop over the equations
      ...
      do j = 3, Ny-2
        do i = 3, Nx-2
          write(11189, '(EN17.5E3,A)', advance="no") prim(ID_EQ,i,j), " " ! WRITE SOL HERE
        end do
      end do
