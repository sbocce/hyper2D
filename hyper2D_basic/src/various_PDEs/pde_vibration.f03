...
!
! ----- EULER EQUATIONS for non-perfect gas (vibration: Harmonic Oscillator)-------
  ...
  ! In this form, we impose two jets, one at the left boundary and one at the bottom boundary.
    ...
    U(4, 1:2, j_jet_start:j_jet_end) = rho0*(uJET**2 + 0.0**2)/2.0 + rho0*compute_e_int(T0) ! total energy
    ...
    U(4, Nx-1:Nx, :) = rho0*(uOUTLET**2 + 0.0**2)/2.0 + rho0*compute_e_int(T0) ! total energy
    ...
    U(4, i_jet_start:i_jet_end, 1:2) = rho0*(0.0**2 + uJET**2)/2.0 + rho0*compute_e_int(T0) ! total energy
    ...
    U(4, :, Ny-1:Ny) = rho0*(0.0**2 + uOUTLET**2)/2.0 + rho0*compute_e_int(T0) ! total energy
