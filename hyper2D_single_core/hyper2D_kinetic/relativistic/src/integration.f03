module integration
  ...
  subroutine compute_fluxes_Mieussens(U, i, j, F_N, F_S, F_W, F_E)
    ...
    real(kind=8) :: x_now, p_now, gam, v_now, F_now, Psi

    x_now = compute_x_from_i(i)
    p_now = compute_p_from_j(j)
    gam   = sqrt(1.0 + p_now*p_now/M/M/c/c)
    gam   = 1.0 ! DBDBDBDBDBDBDBDBDBDBDBDBDBDBDB
    v_now = p_now/gam/M
    F_now = F_field(compute_x_from_i(i))

    ! North flux
    call slope_fun(U(1,i,j-1),U(1,i,j),U(1,i,j+1),U(1,i,j+2), Psi)
    F_N = 0.5*(F_now*U(1,i,j) + F_now*U(1,i,j+1) - abs(F_now)*(U(1,i,j+1) - U(1,i,j) - Psi))

    ! South flux
    call slope_fun(U(1,i,j-2),U(1,i,j-1),U(1,i,j),U(1,i,j+1), Psi)
    F_S = 0.5*(F_now*U(1,i,j-1) + F_now*U(1,i,j) - abs(F_now)*(U(1,i,j) - U(1,i,j-1) - Psi))
